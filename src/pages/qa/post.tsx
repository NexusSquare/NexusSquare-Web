import { Box, Button, FormControl, FormErrorMessage, FormLabel, HStack, Input, Select, Spacer, Text, Textarea, VStack } from "@chakra-ui/react";
...
const Post: NextPage = () => {
    ...
    const { register, handleSubmit, watch, formState: { errors,isSubmitting } } = useForm<QARequest>()
                ...
                <Box>
                    <FormControl isInvalid={errors.category1 !== undefined}>
                        ...
                        <Select required placeholder="カテゴリを選択" {...register('category1')} >
                            {list1.map((category) => {
                                return (
                                    <Box as="option" value={category} key={category}>{category}</Box>
                                )
                            })}
                        </Select>
                    </FormControl>
            ...
            <VStack>
                <Box>
                    <FormControl isInvalid={errors.category2 !== undefined}>
                        ...
                        <FormErrorMessage>
                            {errors.category2 && errors.category2.message}
                        </FormErrorMessage>
                    </FormControl>
                </Box>
            </VStack>

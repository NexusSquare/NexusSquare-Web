...
const Register: NextPage = () => {
    ...
    const GradeList: Function = useCallback(() => {
        ...
        for(let i = 1;i < 5;i++)
            list.push(
                <option key={i} value={i}>{i}年生</option>
            )
        return list
    },[])
        ...
        return (
            <Box as="form" onSubmit={handleSubmit((data) => console.log(`${data.subject}送信完了`))} w={{ base:"100%",sm:"80vw",md:"calc(100vw - 270px)",lg:"50vw" }} paddingTop="20px" alignItems="center">
                ...
                <FormControl isInvalid={errors.department !== undefined}>
                    <Select id="department" placeholder="学部(研究科)を選択" {...register("department",{
                        required: "必須項目です"
                    })}>
                        <DepartmentList />
                    </Select>
                ...
                <FormControl isInvalid={errors.subject !== undefined}>
                    <Select id="subject" placeholder="学科(専攻)を選択" {...register("subject",{
                        required: "必須項目です"
                    })}>
                        <SubjectList />
                    </Select>
                ...
                <FormControl isInvalid={errors.grade !== undefined}>
                    <Select id="grade" placeholder="学年を選択" {...register("grade",{
                        required: "必須項目です"
                    })}>
                        <GradeList />
                    </Select>
